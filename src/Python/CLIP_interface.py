...
def process_image_bread(img, labels):
    global device, model, preprocess
...
def process_tray_bread(tray, labels):
    global input_folder, output_folder
    ...
    for img in os.listdir(path := input_folder+tray+'/food_image/'):
        ...
        values, indices = process_image_bread(path+img, labels)
...
def bread( i : int = None ):
    ...
    if i is None:
        for tray in os.listdir(input_folder):
            process_tray_bread(tray, labels)
    else:
        process_tray_bread("tray"+str(i), labels)

def plates( i : int = None ):
